...
struct CompanionTests {
    @Test func companionReturnsMessageForEarlyMorning() throws {
        ...
        let (message, scheduledHour) = try sut.nextMotivationalMessage(
            weekday: 0,
    ...
    @Test func messageOnSuitableDay() throws {
        let messageDetails = [
            CompanionMessage(
                message: "Some advice for Tuesday", timeOfDay: .earlyMorning, validDay: .tuesday)
        ]
        let sut = Companion(
            messageDetails: messageDetails, notificationSender: StubbedNotificationSender())
        #expect(throws: CompanionError.NoValidMessages) {
            let (_, _) = try sut.nextMotivationalMessage(
                weekday: 0,
    ...
    @Test func messageOnWeeklyWeightLossScenario() throws {
        let messageDetails = [
            CompanionMessage(message: "You've lost some weight", validScenario: .weeklyWeightLoss)
        ]
        let sut = Companion(
            messageDetails: messageDetails, notificationSender: StubbedNotificationSender())
        #expect(throws: CompanionError.NoValidMessages) {
    ...
    @Test func messageOnMonthlyWeightLossScenario() throws {
        let messageDetails = [
            CompanionMessage(message: "You've lost some weight", validScenario: .monthlyWeightLoss)
        ]
        let sut = Companion(
            messageDetails: messageDetails, notificationSender: StubbedNotificationSender())
        #expect(throws: CompanionError.NoValidMessages) {
    ...
    @Test func messageOnWeeklyWeightGainScenario() throws {
        let messageDetails = [
            CompanionMessage(message: "You've gained some weight", validScenario: .weeklyWeightGain)
        ]
        let sut = Companion(
            messageDetails: messageDetails, notificationSender: StubbedNotificationSender())
        #expect(throws: CompanionError.NoValidMessages) {
    ...
    @Test func schedulesNotificationForTomorrow() async throws {
        ...
        try await sut.scheduleTomorrowsMotivationalMessage(
            context: .init(
        ...
        #expect(await sut.notificationSender.hasPendingRequests())
        let currentWeekday = Calendar.current.dateComponents([.weekday], from: dateFromComponents())
            .weekday!
        #expect(currentWeekday == 2)
        let scheduledWeekday = (sut.notificationSender as? StubbedNotificationSender)?.requestDates
            .first?.weekday
    ...
    private func dateFromComponents() -> Date {
        let dc = DateComponents(
            calendar: Calendar.current, year: 2024, month: 1, day: 1, hour: 11, minute: 30)
        return dc.date!
    }
