...
extension HKHealthStore: HealthStore {
    ...
    public func authorize() async throws {
        if HKHealthStore.isHealthDataAvailable(),
            ...
            let basalEnergyBurned = HKObjectType.quantityType(
                forIdentifier: HKQuantityTypeIdentifier.basalEnergyBurned),
            let activeEnergyBurned = HKObjectType.quantityType(
            ...
            let weight = HKObjectType.quantityType(forIdentifier: HKQuantityTypeIdentifier.bodyMass)
        {
            try await requestAuthorization(
                toShare: [dietaryEnergyConsumed, activeEnergyBurned, weight],
    ...
    private func countForType(_ typeIdentifier: HKQuantityTypeIdentifier, date: Date) async throws
        -> Int
    {
    ...
    {
        let predicate = HKQuery.predicateForSamples(
            withStart: fromDate, end: toDate, options: .strictStartDate)
        return try await caloriesConsumedAllDataPoints(
            predicate: predicate, applyModifier: applyModifier)
    }

    private func dataPointsForType(
        _ typeIdentifier: HKQuantityTypeIdentifier,
        predicate: NSPredicate?,
        modifierFactor: Double
    ) async throws -> [(Date, Int)] {
        guard let type = HKObjectType.quantityType(forIdentifier: typeIdentifier) else {
            return []
        }
        ...
        return await withCheckedContinuation { continuation in
            let query = HKSampleQuery(
                ...
                guard let result = result,
                    let dataPoints = result as? [HKCumulativeQuantitySample]
                else {
                    continuation.resume(returning: [])
                    return
                }
                continuation.resume(
                    returning: dataPoints.map {
                        let date = $0.startDate
                        let calories = Int(
                            $0.quantity.doubleValue(for: .kilocalorie()) * modifierFactor)
    ...
    private func caloriesConsumedAllDataPoints(
        predicate: NSPredicate?,
        applyModifier: Bool
    ) async throws -> [(Date, Int)] {
        try await dataPointsForType(
    ...
    public func bmrBetweenDates(
        ...
        let predicate = HKQuery.predicateForSamples(
            withStart: fromDate, end: toDate, options: .strictStartDate)
        return try await dataPointsForType(
            .basalEnergyBurned,
            predicate: predicate,
    ...
    public func weightBetweenDates(fromDate: Date, toDate: Date) async throws -> [(Date, Int)] {
        ...
        return await withCheckedContinuation { continuation in
            let query = HKSampleQuery(
                ...
                guard let result = result,
                    let dataPoints = result as? [HKQuantitySample]
                else {
                    continuation.resume(returning: [])
                    return
                }
    ...
    public func weightChange(days: Int) async throws -> Int {
        ...
        guard let first = weightDataPoints.first,
            let last = weightDataPoints.last
        else {
            return 0
        }
    ...
    public func addFoodEntry(_ foodEntry: FoodEntry) async throws {
        ...
        let caloriesConsumed = HKQuantitySample(
            type: caloriesConsumedType,
            quantity: quantity,
            start: timeConsumed,
            end: timeConsumed,
    ...
    public func deleteFoodEntry(_ foodEntry: FoodEntry) async throws {
        ...
        return await withCheckedContinuation { continuation in
            let predicate = HKQuery.predicateForSamples(
                withStart: timeConsumed, end: timeConsumed.addingTimeInterval(1))
            let query = HKSampleQuery.init(
                sampleType: caloriesConsumedType,
                predicate: predicate,
                limit: HKObjectQueryNoLimit,
                sortDescriptors: nil
            ) { [weak self] (query, results, error) in
                guard
                    let result = (results as? [HKQuantitySample])?.first(where: { result in
    ...
    public func addExerciseEntry(_ exerciseEntry: ExerciseEntry) async throws {
        ...
        let quantity = HKQuantity(
            unit: .largeCalorie(), doubleValue: Double(exerciseEntry.calories))
    ...
    public func addWeightEntry(_ weightEntry: WeightEntry) async throws {
        ...
        let newBodyMass = HKQuantitySample(
            type: bodyMassType,
            quantity: weight,
            start: timeRecorded,
            end: timeRecorded,
