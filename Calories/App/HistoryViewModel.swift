...
class HistoryViewModel {
    ...
    func fetchDaySections(forDate dateForEntries: Date) {
        let weekPrior: Date = Calendar.current.startOfDay(for: dateForEntries).addingTimeInterval(
            -Double(secsPerWeek))
        let foodEntriesForWeek =
            modelContext?.foodResults(for: #Predicate { $0.timeConsumed >= weekPrior }) ?? []
